...
* **A type the server will not be able to describe.** A float16 column reads back as ``FLOAT``
  ...
  Parquet to record at all. See :func:`mikro.vocabulary.duckdb_type`, which maps only what
...
from mikro.vocabulary import duckdb_type

if TYPE_CHECKING:
    import pyarrow as pa

    from mikro.api.schema import ColumnInput
...
def arrow_schema_of(frame: object) -> pa.Schema:
    ...
    This is the same conversion :func:`mikro.io.upload._parquet_payload`
