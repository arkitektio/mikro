...
def test_create_folder(deployed_app: DeployedMikro) -> None:
    ...
    folder = deployed_app.mikro.create_folder(name="creatable_folder")
...
def test_create_folder_in_parent(deployed_app: DeployedMikro) -> None:
    ...
    parent = deployed_app.mikro.create_folder(name="parent_folder")
    child = deployed_app.mikro.create_folder(name="child_folder", parent=parent.id)
...
def test_get_folder(deployed_app: DeployedMikro) -> None:
    ...
    created = deployed_app.mikro.create_folder(name="fetchable_folder")
    fetched = deployed_app.mikro.get_folder(id=created.id)
...
def test_update_folder(deployed_app: DeployedMikro) -> None:
    ...
    created = deployed_app.mikro.create_folder(name="folder_before_rename")
    updated = deployed_app.mikro.update_folder(id=created.id, name="folder_after_rename")
...
def test_search_folders(deployed_app: DeployedMikro) -> None:
    ...
    created = deployed_app.mikro.create_folder(name="searchable_folder_xyzzy")
    results = deployed_app.mikro.search_folders(search="searchable_folder_xyzzy")
